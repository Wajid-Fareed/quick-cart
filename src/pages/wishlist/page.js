...
const Wishlist = () => {
  const { wishlist, setWishlist, cart,
    setCart } = useContext(CartContext);
  ...
  return (
    <Container className="py-9">
      ...
      {wishlist.lenght > 0 ? (<div className='min-w-full overflow-x-auto'>
        ...
        {wishlist.map((item) => (
          <div className='flex justify-between items-center py-4 border-b gap-10 min-w-[975px]' key={item.id}>
            ...
            <div>
              <button className='bg-black text-white text-xl font-semibold w-52 h-14 flex justify-center items-center rounded-md' onClick={() => handleAddToCartFromWishlist(item)} >Add to Cart</button>
            </div>
        ...
        <div className='flex flex-col justify-center items-center gap-5 h-[73vh]'>
          <h2 className='text-2xl font-medium'>Wishlist is empty.</h2>
          <Link to='/' className='bg-black text-white rounded-md w-40 h-10 flex justify-center items-center' > Back to Home</Link>
        </div>
      )}
